...
int HcclOpBaseBrocastTest::init_buf_val()
{
    ...
    sprintf(bin_path, "/root/Workdir/hccl_test/log/zhangdx_init_rank_%d.bin", rank_id);
...
int HcclOpBaseBrocastTest::check_buf_result()
{
    ...
    char bin_path[MAX_PATH_LEN];
    memset(bin_path, 0, MAX_PATH_LEN);
    sprintf(bin_path, "/root/Workdir/hccl_test/log/zhangdxt_check_rank_%d.bin", rank_id);
